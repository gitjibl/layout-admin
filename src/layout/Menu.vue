<template>
  <el-menu
    ...
    text-color="#fff"
    active-text-color="rgb(21 209 113)"
    mode="horizontal"
    unique-opened
  >
    <!--手动定义菜单-->
    ...
    <template v-for="(route, index) in permission_routes">
      <template v-if="!route.hidden">
        ...
        >
          <app-link
            :to="resolvePath(route.path, route.children[0].path)"
            style="display: block"
          >
            <template
              v-if="route.children[0].meta && !route.children[0].meta.imgUrl"
            >
              <item
                v-if="route.children[0].meta"
            ...
            <template v-else>
              <el-image
                :src="route.children[0].meta.imgUrl"
        ...
        <el-submenu :index="route.path" :key="index" v-else>
          ...
          <template v-else slot="title">
            <el-image
              :src="route.meta.imgUrl"
              style="height: 60px; width: 60px"
            ></el-image>
          </template>
          <MenuItemGroup
            v-for="(e, i) in route.children"
            :key="i"
            :item="e"
